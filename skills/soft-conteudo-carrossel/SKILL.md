...
> Se existe skill de voz destilada do cliente (uma skill `soft-voz-*` própria dele), ela é a fonte do tom: pilares, bordões e anti-valores. Você já leu ela no Passo 0 pra tirar os **padrões de ausência**; aqui usa o lado positivo (pilares, bordões, ritmo) pra escrever COMO ele fala, enquanto a lista de ausência guarda o que ele nunca faz.
