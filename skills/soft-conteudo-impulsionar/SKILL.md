...
## As duas esteiras (produção + escala): só se PROCESSUALIZA depois de achar o vencedor
Escalar tráfego de forma sustentada não é um criativo genial, são DUAS pipelines rodando em paralelo, sempre com contingência de oferta:
- **Esteira de criação (o volume que alimenta a conta):** uma meta SEMANAL de N criativos novos. Começa pequeno (~5 a 10 por semana) e sobe conforme o perfil aguenta produzir. O volume é o que dá matéria-prima pro algoritmo achar o vencedor; um criativo solto por semana não escala nada. (Quem ESCREVE/produz cada peça é a `soft-conteudo-carrossel/-reels/-headlines` e a `soft-designer`; aqui a esteira define a CADÊNCIA e o quê priorizar.)
- **Esteira de otimização/escala (o que fazer com o vencedor):** achou o criativo que responde, não para nele, extrai variações em ordem de alavanca: **formato** (mesma mensagem em outro cenário/enquadramento) → **aberturas** (mesma peça, ganchos diferentes) → **ângulos primos** (o mesmo tema por um ângulo vizinho) → **empilhamento de ganchos** (mais de uma abertura antes de entregar a mensagem). Cada camada é uma aposta barata em cima do que já provou.
- **Contingência de oferta (a esteira de ofertas):** nunca depende de uma oferta só. Mantém uma oferta reserva pronta pra entrar quando a principal fadiga ou o custo sobe, do mesmo jeito que troca de criativo.

**O princípio que rege as duas:** você só PROCESSUALIZA depois de achar o que funciona. Primeiro itera solto (testa formatos, ganchos e ofertas sem sistema), acha o que o mercado responde, e SÓ ENTÃO transforma o vencedor em esteira repetível. Montar processo antes de ter vencedor é industrializar o palpite. (Lastro genérico: o princípio de descobrir o que funciona por iteração e só depois repetir, apostar barato em muitas tentativas e dobrar na que responde, é a lógica que Naval resume.)
