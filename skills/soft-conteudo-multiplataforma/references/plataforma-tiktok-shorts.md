# Plataforma TikTok / Reels / YouTube Shorts

Reel Lo-fi Soft **já é o ponto de partida**. Essa reference cobre os ajustes que cada plataforma pede - não recria o reel do zero, adapta.

## Regras universais 2026 (valem pras 3)

1. **Intro retention 70%+ no primeiro 3s** é o benchmark. Abaixo disso, algoritmo suprime.
2. **15-35s é a faixa ideal.** YouTube Shorts agora aceita até 3min, mas alta performance ainda fica em 15-35s.
3. **Legendas sempre.** 60%+ assiste mudo.
4. **Formato 9:16 vertical.** Nunca horizontal, nunca quadrado.
5. **Gancho visual + texto + verbal nos 3 primeiros segundos.** Redundância tríplice = +3x retenção.
6. **Loop natural** - o final conecta com o começo = rewatch = boost algorítmico.
7. **Zero intro longa de "oi pessoal".** Começa em medias res.

---

## Diferença entre plataformas (ajuste fino)

| Elemento | TikTok | Instagram Reels | YouTube Shorts |
|---|---|---|---|
| Audio trending | **Crítico** - FYP depende disso | Importante | Menos crítico |
| Texto sobreposto | Muito comum, parte da linguagem | Importante | Importante |
| Hashtags | 3-5 nicho específicas | 3-5 mistas | Menos, SEO no título importa mais |
| CTA | Resposta nos comentários | Resposta ou Direct | "Canal pra full breakdown" |
| Legenda/descrição | Curta, tipo gancho | Curta, pode ser gancho | **SEO matters** - 1ª linha decide click |

**Para Soft:** o reel original tem CTA pra Carta via "Direct PALAVRA". No TikTok/Shorts, adapta pra "resposta nesse vídeo com PALAVRA" ou "link na bio". O núcleo não muda.

---

## Adaptação do Reel Lo-fi Soft

**Passo 1 - Verifica se o reel original já tem:**
- [ ] Gancho nos primeiros 3s
- [ ] Problema Sofisticado nomeado
- [ ] Método com nome próprio
- [ ] CTA filtrante

Se sim, adapta só o invólucro (legenda + hashtag + áudio). Se não, reescreve o reel primeiro (skill `soft-conteudo-reels`).

**Passo 2 - Ajuste de invólucro por plataforma:**

### TikTok
- Áudio trending relevante ao nicho (se tiver)
- Hashtags: `#[nicho]` + `#[subnicho]` + `#fyp` + `#[problema]`
- Legenda: 1 linha curta (o próprio gancho)
- CTA: "comenta [PALAVRA] abaixo que eu respondo"

### Instagram Reels
- Mantém como já tá na peça de origem (soft-conteudo-*)
- Áudio: original ou trending sem música em foco
- Hashtags 3-5
- CTA: "Direct [PALAVRA]"

### YouTube Shorts
- **Título SEO**: termo de busca + ângulo contraintuitivo
- Descrição: primeira linha é pitch curto, depois contexto
- Hashtags 2-3
- CTA: "link na bio" ou "canal pra breakdown completo"
- **Thumbnail customizada** (opcional mas poderosa)

---

## Estrutura de 30s Soft (template)

```
[0-3s] GANCHO
Visual: [ação concreta + texto sobreposto 5-8 palavras]
Fala: [contradição forte, 1 frase]

[4-10s] PROBLEMA SOFISTICADO
Visual: [mudança de enquadramento]
Fala: [inimigo-categoria nomeado + efeito colateral]

[11-20s] MÉTODO
Visual: [mudança + texto sobreposto com nome do método]
Fala: [método nomeado + 1 diferencial chave]

[21-28s] PROVA/REFRAME
Visual: [mudança]
Fala: [número, case, ou reframe final]

[29-30s] CTA + LOOP
Visual: [volta ao enquadramento do 0s - loop]
Fala: [CTA curto]
```

---

## Cauda de publicação TikTok/Shorts/Reels

Entrega junto com a peça:

1. **Legenda/descrição curta** pronta.
2. **3-5 hashtags** específicas.
3. **Sugestão de áudio** (se aplicável).
4. **Thumbnail** (só YouTube Shorts) - frase 5-8 palavras.

---

## Checklist TikTok/Shorts/Reels

- [ ] Gancho nos primeiros 3s (visual + texto + verbal)?
- [ ] 15-35s de duração?
- [ ] 9:16 vertical?
- [ ] Legendas presentes?
- [ ] Método nomeado aparece?
- [ ] CTA filtrante no final?
- [ ] Loop natural?
- [ ] Invólucro adaptado pra plataforma específica?

Tudo sim → entrega.
