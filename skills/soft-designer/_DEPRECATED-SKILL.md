...
> **Molde carrossel de feed 1080x1350 (uma cor accent + watermark textural):** para carrossel de feed vertical com fundo preto, uma cor accent do cliente e watermark textural de nicho, usa `references/carrossel-feed-arquitetura.md` (regras visuais) + `references/carrossel-feed-template.py` (template Python parametrizado, roda com Playwright). **A quantidade de slides (6-9) e a alocação de imagem IA são DECIDIDAS pela skill de copy slide-a-slide, não fixadas no template.** Todo parâmetro específico do cliente (cor, handle, palavras do nicho, palavra-chave do CTA) vive no bloco `USER_*` do template.
