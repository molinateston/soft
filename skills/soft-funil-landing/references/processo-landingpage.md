...
Escreve todos os blocos em sequência, num documento markdown (arquivo `.md` salvo no disco; se o ambiente renderizar markdown, mostre), com:
...
A auditoria Blair é PRÉ-FILTRO: qualquer movimento ausente, reforça antes do Crivo, não auto-aprova. O veredito final é do Crivo (pior bloco manda).
...
3. **Gate CUB bloqueante + as 3 perguntas do Harry** (`crivo/03-gate-cub.md`): monta a tabela por dentro, o veredito é o pior bloco, peça que falha não sai, volta pra reescrita.

O anti-IA limpa o robô; o Crivo dá a força. Limpo não é forte. Os dois, nessa ordem.

**O Crivo roda por dentro e a tabela NUNCA vai pra saída** (vale o SKILL.md, Ação 5). O que acompanha a entrega é um **relato curto de 3 a 6 linhas** dizendo o que o gate conferiu: ancoragem (qual fala real sustentou a promessa, com N ou `[N A CONFIRMAR]`), o pior bloco e por quê, o que foi reescrito, e o que ficou marcado como pendente. Sem esse relato a peça não foi entregue; a tabela crua fica de fora.
