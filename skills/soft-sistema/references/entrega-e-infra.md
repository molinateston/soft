...
- **Commit por escopo**: se o ambiente tiver delegação, cada agente do Build commita o escopo dele; senão você commita escopo por escopo em sequência (mensagem clara do que fez). Facilita o review e o rollback por peça.
...
- Token: `GITHUB_PAT` no cofre de credenciais do dono (arquivo `.env` fora do repo). `gh` autentica por ele.
...
- Segredo **nunca** em texto puro no repo. Por ora, o cofre é um **`.env` cifrado no servidor**, fora do repo (um pro operador, no home dele; um por serviço, ao lado do serviço). Upgrade pra gerenciador de senha depois.
