...
description: >-
  Gera contratos de prestação de serviço prontos pra assinar, em português
  brasileiro: consultoria, mentoria, serviço feito-pra-você, infoproduto e
  qualquer venda de serviço fechada pelo dono. Dois modos: SOFT ENXUTO
  (default, 11 cláusulas, tom flexível, sem multa) e ROBUSTO (13 cláusulas,
  anti-calote completo com multa, juros, protesto). Cobre 4 formatos
  (mentoria 1:1, em grupo, infoproduto gravado, feito-pra-você) e 4
  modalidades de pagamento. Atende PF ou PJ. Use SEMPRE que envolver
  "contrato de serviço", "contrato de consultoria", "contrato de mentoria",
  "cláusulas", "preciso de contrato", inclusive quando o usuário descrever o
  caso sem citar "contrato" diretamente. Esta skill gera o CONTRATO que
  formaliza a venda já fechada; a proposta que antecede e apresenta a oferta
  é soft-vendas-proposta.
---

# Soft Vendas · Contratos

Gera contrato de prestação de serviço pronto pra assinar: consultoria, mentoria, feito-pra-você, infoproduto ou qualquer venda de serviço fechada. Os templates base nasceram de consultoria/mentoria e se adaptam ao serviço descrito na entrevista de escopo.
...
- Modelo padrão do método (consultoria de implementação, micro treinamento, etc.)
...
Sempre que o pedido envolver **gerar, revisar ou adaptar contrato de prestação de serviço**: consultoria, mentoria, serviço feito-pra-você, infoproduto ou outra venda de serviço. Inclui pedidos diretos ("preciso de um contrato") e indiretos ("fechei cliente, como protejo o recebimento?").
...
Salvar em `/mnt/user-data/outputs/contrato-[nome]-[data].docx` e apresentar com `present_files` (Claude Chat).

**Fallback (ambiente sem `/mnt/user-data/outputs`):** se o diretório de saída não existir (rodando fora do Claude Chat, ex. CLI/bot/Code), não tenta gravar `.docx` nem chamar `present_files`: entrega o contrato como **markdown estruturado inline**, pronto pra copiar pro D4Sign/Clicksign/Autentique (no Code, pode gravar o `.md`/`.docx` na working dir). O conteúdo das cláusulas é o mesmo; muda só o formato de entrega.
...
- Nunca usar linguagem de marketing Soft no corpo do contrato. nome de método/programa só aparece como nome próprio do programa do dono, não como conceito
