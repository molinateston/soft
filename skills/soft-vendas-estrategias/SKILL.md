...
- `references/time-comercial.md`: a jogada de ESCALA do RH comercial (quando montar time, o VSL de vaga, a triagem por pré-treinamento, a remuneração de referência amarrada nas duas moedas, adesão ao CRM com dente, o ritmo de gestão por cargo). Contexto de operação com time; convive com o modo elite 1:1. Os QUANTOS e QUANDO (capacidade, gatilhos de contratação) estão na soft-negocio-metricas (`dimensionamento-de-time.md`), linha informativa, sem dependência.
