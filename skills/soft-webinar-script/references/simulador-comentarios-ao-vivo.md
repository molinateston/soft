...
> **A decisão que abre esta reference (autor do método, 10/jun):** no perpétuo, a **simulação de comentários É padrão.** Isto SUPERA a nota mais conservadora do `gravacao-energia-ao-vivo.md` (que tratava só comentários reais como padrão e a simulação como variante a validar). O autor do método decidiu: a sala simulada é o desenho operacional do perpétuo Soft. A nota de honestidade fica LEVE e precisa: a **escassez/vagas declaradas continuam REAIS** (guarda-corpo G7, sessão de lugares limitados de verdade, contador honesto); os **comentários reproduzem a energia típica que uma sala real daquele tamanho teria** naquele momento da aula. Não se inventa resultado de cliente, não se inventa número de vagas, não se mente estoque. Simula-se a CONVERSA de uma sala, não a PROVA do produto.
...
- 2. O formato Excel (a skill aprende o modelo do autor do método)
...
**FORMATO CANÔNICO da plataforma do autor do método (confirmado 10/jun).** É ESTE o CSV de import:
...
> **Outras plataformas:** se um player usa EverWebinar/WebinarKit com colunas diferentes, vale a régra geral (pedir o modelo, gerar naquele formato). Mas `username,message,minutes,seconds` é o formato do autor do método e o **default da skill**.
...
- **Nomes BR variados:** alternar regiões e gerações. EXEMPLO de pool: Camila, Rafael, Patrícia, Marcos, a autora do case de arquitetura, o autor do case de odontologia, Fernanda, Tiago, Aline, Rodrigo, Vanessa, Diego, Larissa, Anderson, Gabriela, Luan, Bia, Wesley, Sandra, Eduardo. Evitar repetir o mesmo nome em comentários próximos no tempo. NÃO usar nomes que o host vá ecoar a menos que o eco esteja casado (ver §6).
...
Pré-requisito: roteiro/deck pronto (ou pelo menos a ordem dos blocos e os comandos do host) + N (pessoas-alvo) + o modelo de planilha do autor do método.
...
- [ ] **Formato:** a planilha está no formato EXATO do modelo do autor do método/plataforma (colunas, ordem, cabeçalho, formato de timestamp). Default só foi usado se não havia modelo, e isso foi avisado.
...
- **Formato chutado.** Gerar num formato que a plataforma do autor do método não importa, obrigando remapeamento manual. Sempre o modelo dele.
