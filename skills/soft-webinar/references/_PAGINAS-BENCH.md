# Páginas de Webinar: BENCHMARK das páginas reais do caso estudado

> **Pra que serve:** modelo reutilizável das 3 páginas que orbitam o webinar (CAPTURA → OBRIGADO → CHECKOUT), extraído das páginas REAIS do corpus estudado que estão no ar, não da teoria. Use como régua quando montar as páginas de qualquer cliente.
>
...
## A régua que governa as 3 páginas (direto do corpus estudado)
...
> **Mito que o bench desfaz:** *"já testei páginas lindas, não converteu."* A página do corpus estudado é "feia" e converte 30%. Cada animação é uma fricção. **Página feia que roda ganha de página linda que enfeita.**

> **Marca-neutra:** o ID visual abaixo é o DO CASO ESTUDADO (bench), não um padrão a copiar. A pele do dono/cliente (cor, fonte, marca) vem da config dele; o que se modela daqui é a PREMISSA (1 cor de ação, contraste, mobile-first), nunca a paleta.

**Premissas do ID visual (o que se modela, sem copiar paleta):** fundo de alto contraste, UMA única cor de ação, tipografia display pra títulos + uma fonte de corpo + uma mono pra labels/eyebrows, cantos sóbrios, linhas finas discretas. Eyebrow = label mono na cor de ação com tracinho antes. Cor, fontes e marca vêm do modelo do dono via config. Mobile-first é a régua (70-85% do tráfego é celular).
...
O autor do corpus usa a bio detalhada em **dois formatos**: escolha pelo tipo de página:
...
- **Empatia/cicatriz ANTES do feito.** Sempre. É a ordem que o autor do corpus bate.
...
- Eyebrow com check na cor de ação: `✓ Inscrição confirmada`
- H1 display gigante: **"Informações importantes"**
- Linha-lead com nome dinâmico: *"Tá garantida, **[nome]**. O acesso é **gratuito**[, e limitado a (DO DONO: limite real e motivo), só se houver]. Leia com atenção pra não ficar de fora."*
...
Três cards com X vermelho, lado a lado, que justificam por que NÃO dá pra deixar pra depois. Cada card só existe com a decisão do dono no insumo (replay, material, regra de saída); sem ela, o bloco sai e a pergunta vai ao dono:
```
✗ [DO DONO: política de replay, se ele decidiu]
✗ [DO DONO: política de material, se ele decidiu]
✗ [DO DONO: regra de saída, se ele decidiu]
```
...
Um card escuro com cabeçalho na cor de ação "SUA AULA", contendo:
...
- **Adicione na agenda:** 3 botões (Google · Outlook · .ICS pra Apple e outros), com o evento na data/hora exatas e link individual embutido. Atrito zero.

### Bloco 4: Gate "Passo decisivo" → a FICHA (a mina de ouro)
Card destacado na cor de ação que empurra pra ficha:
- Selo: `Passo decisivo`
- H2: **"Prepara a tua aula"**
- Texto: *"[DO DONO: limite real por sessão e o motivo, só se houver.] Preenche a ficha rápida (3 min). Eu uso tuas respostas pra preparar a sala pro teu caso."*
- Botão largo na cor de ação: **"Responder a ficha →"** → leva pra `/ficha/`
...
Card discreto, só com o presente real que o dono confirmou: *"🎁 [DO DONO: o presente de retenção e quando sai]. Entra no horário e fica até o final."* Sem presente no insumo, o bloco sai.
...
- **Insight (opcional):** "O que mais te emperra hoje?" (campo aberto, na voz do lead).
- **Tela final:** *"Pronto, [nome]. Inscrição confirmada."* + **único próximo passo = botão WhatsApp** com mensagem pré-pronta: *"Oi o autor do corpus! Acabei de preencher minha ficha da masterclass. Quero confirmar minha presença."* + nota: *"A sala abre no horário que você escolheu · não falta[, (presente real do dono), só se houver]."*

## O fluxo de obrigado, resumido
CONFIRMA (nome) → ANTI-FUGA (3 badges) → SUA AULA (data/hora + timer + agenda) → FICHA (gate destacado) → opt-in WhatsApp (na tela final da ficha) → PRESENTE. O WhatsApp é o destino final, não o formulário.
...
> **O pedido do corpus estudado (a régua deste item):** checkout só com **cronômetro + "você está entre os N primeiros" + garantia + provas + bônus. NADA mais.** (cronômetro e N primeiros só com o prazo e o limite reais do dono) Sem FAQ longo, sem re-explicar o método, sem dobra de vendas. Quem chegou aqui já está com o cartão na mão. Cada bloco extra é fricção que faz a pessoa pensar de novo.
...
- Visual: faixa fina na cor de ação no topo, número grande em fonte display, mono no label. Acima da dobra, sempre visível.

### Bloco B: "Você está entre os N primeiros" (só com o limite real do dono + prêmio do link)
- Selo/linha logo abaixo do timer, só quando o insumo traz o limite e o bônus: **"Você está entre os [N do dono] primeiros: bônus liberado."**
- Vem do mecanismo de **link controlado** do corpus estudado: o checkout é um link próprio, recompensado, desviado do site sem oferta. *"Quem usar o link que eu vou disponibilizar AQUI ganha [vantagem]."* O checkout vira **prêmio** (a "telinha que confirma os 15 primeiros"), não uma página passiva.
...
- (Webinar perpétuo do corpus estudado fixa 90+90; GA fecha sem garantia. O desenho é `[A CONFIRMAR]` por cliente.)
...
- ❌ FAQ longo de 5-7 perguntas (isso é checkout "completo" da reference; o autor do corpus pediu enxuto).
...
## Números de referência de máquina bem montada (corpus)
