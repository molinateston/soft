# O gate das páginas do webinar, linha a linha (P7)
...
| **Checkout ENXUTO (5 blocos, nada mais)** | só: cronômetro (só com prazo real do dono) + "N primeiros" (só com o limite real do dono; sem ele, o bloco sai) + garantia (cardápio) + provas (na moeda) + bônus/stack (cada item mata 1 objeção + soma riscada) + pagamento. SEM FAQ, SEM re-explicar método, SEM upsell, SEM bio. Qualquer bloco extra = ✗ | |
...
| **Anti-IA (HARD)** | zero travessão "," · zero verbo-freio banido (exceção: aspa literal do cliente) · sem frase-emoldura ("a verdade é", "o segredo") · sem verbo-clichê ("revoluciona, transforma"). **No chat (sem o lint), faz CTRL+F manual de "," e do verbo-freio banido antes de marcar ✓.** No Code, roda `python3 scripts/lint_copy.py`. | |
