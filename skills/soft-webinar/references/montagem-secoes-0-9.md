...
- Seção 3, O DIAGNÓSTICO + O MECANISMO (3a diagnóstico · 3b mecanismo · 3c batida de 4 tempos)
- Seção 4, A PERSONA-ÂNCORA + crenças rotuladas
- Seção 5, As PROVAS + a PROVA EXTERNA
- Seção 6, A CONTA · Seção 7, O FORMATO
- Seção 8, O PITCH (coreografia de fechamento)
- Seção 9, INSUMOS ORGANIZADOS
...
> **Ordem canônica de revelação da oferta** (afunda a pessoa, idêntica em Enes/gestão ágil/o autor do corpus): transição+depoimento+"EU QUERO" → caminho 1 (sozinho) x 2 (comigo) → PUV → módulos por benefício → inclusos de confiança → âncora alta → prateleira com preço → bônus-âncora>produto → bônus Mastercard → garantia → queda em degraus → parcela/dia → **15-primeiros em camadas + bônus surpresa** → recap total no clique → 3 CTAs + rede. (Verbatim: `references/ancoragem-e-fechamento.md`, `references/stack-de-oferta-e-bonus.md` §10, `references/exemplos-por-bloco/`.)

## Seção 9, INSUMOS ORGANIZADOS (FINAL, sempre, peças soltas reutilizáveis)
A que o autor do corpus lê primeiro. NÃO resumo: a matéria-prima das seções **destilada em peças soltas**, cada uma pronta pra copiar pra página/anúncio/e-mail/roteiro. Sai SEMPRE (com `[A CONFIRMAR]` no que faltar):
