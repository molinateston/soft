...
> **Quando consultar (dentro da Etapa MENSAGENS (desta skill)):** no **Passo 1**, depois de o especialista dizer se a régua é AO VIVO ou PERPÉTUO. É aqui que mora a diferença de TIMING entre os dois modos (datas absolutas vs relativas ao cadastro), a **escassez por SESSÃO** que o perpétuo usa no lugar da escassez de evento, o **multi-horário** (que muda a cadência das mensagens), o **link individual por lead** (o que faz o perpétuo parecer ao vivo) e as **métricas do perpétuo já no ar** (pra diagnosticar antes de culpar a copy).

> **Fronteiras (o que NÃO mora aqui, manda pra skill certa):**
> - **A mecânica crua do corpus estudado** (validar ao vivo, 150-200 valida / 8-15 grava, ~10% de sinal verde, sala = sessão estratégica, ROAS-rei, otimizar tráfego pra venda, escala horizontal, zerar ROAS, high ticket não fecha sozinho): `perpetuo-mecanica.md` (a reference irmã, copiada junto nesta skill). As falas literais do corpus estudado vivem lá.
> - **Gravar/editar o vídeo, conduta de sala, chat semeado, setup técnico, plataforma (EverWebinar × WebinarKit) e edição** → **Etapa AULA (desta skill)** / **soft-webinar** (pacote). Esta skill só escreve a COPY das mensagens; não grava, não edita, não configura player.
> - **Oferta/escassez/Q&A na AULA** → **Etapa PLANO/OFERTA (desta skill)** e **Etapa AULA (desta skill)**. Aqui a escassez aparece só como o que a MENSAGEM pode dizer (eco da escassez da sessão, nunca uma nova).
> - **Páginas de cadastro/obrigado/checkout** → **soft-funil-landing**.
> - **Montagem da máquina de pós** (tags, marcos de %, roteamento de CRM) → **Etapa MENSAGENS (desta skill)**.

> Destilado de: corpus estudadocap 13; corpus estudado("Perpetuar"). Falas literais marcadas com origem. Exemplos de nicho alheio marcados EXEMPLO, didáticos, nunca pra decalcar.
...
### O padrão Soft, da fala do corpus estudado: 4 horários por dia
...
|---------|---------------------------|-----------------|-------------|
| Cadastrados → compareceram | **30-40%** (fala do corpus estudado, varia com a época) | Abaixo de 30% | sequência PRÉ fraca: revisar e-mail/WhatsApp e a promessa da captura, **e conferir se o WhatsApp está ligado** (é a maior alavanca de comparecimento) |
| Tempo médio de retenção | 50-70% | Abaixo de 45% | algum bloco da AULA vaza (não é a mensagem) → Etapa AULA (desta skill) |
| Comparece → compra | **~6% piso de saúde** (médio ticket) | Abaixo disso | "ou o COP está errado, ou o ticket não fecha a conta" (corpus) → oferta/aula |
| ROAS dos anúncios | **2,2-4** | Abaixo de 1,5x | anúncio/público/pixel → soft-trafego-meta |
