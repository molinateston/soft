---
name: soft-apostila
description: "Transforma uma aula/live gravada (Zoom, YouTube, MP4) em MATERIAL navegável, uma apostila/handout. Pipeline Whisper + Claude: extrai áudio, transcreve, limpa muletas, segmenta em capítulos e enriquece cada um (vira texto corrido, não transcrição crua). Output adapta ao ambiente: no chat entrega em Markdown; no Code renderiza a apostila HTML single-file (sidebar com índice, navegação, scroll-spy) e publica. Marca-neutra: tema/cor/ID vêm do dono. Use quando o dono quiser virar conteúdo gravado em material, bônus de webinar, isca, micro-aula do funil, módulo de mentoria. NÃO use para roteiro/ideia de conteúdo de feed (soft-conteudo), nem edição de vídeo (soft-editor-video)."
---
