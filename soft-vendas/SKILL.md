---
name: soft-vendas
description: A venda consultiva do método Soft, o fechamento limpo. Transforma a conversa em cliente sem empurrar, revela a dor real e pede a decisão. O canal padrão do 1:1 é a DM/WhatsApp (fecha no chat com áudio/doc/vídeo); call/reunião é exceção de contexto (ticket muito alto, lead pede, B2B); SDR+Closer só com equipe e volume. Cobre script de venda, prospecção no Direct, isolamento de objeção, diagnóstico de conversa empacada, copiloto em tempo real, métricas de pipeline e pós-venda. Puxa a Oferta e a PUV do Plano. Use SEMPRE que envolver "script de venda", "venda", "vender", "objeção", "fechamento", "follow-up", "prospecção", "DM/Direct", "WhatsApp", "vender na DM sem call", "fechar high-ticket no chat", "sem sessão estratégica", "lead sumiu", "lead frio", "não consigo cobrar caro", "cabeça do vendedor", "pipeline", "win rate", "CAC", "analisa essa conversa", "pós-venda", "indicação", "testemunho". NÃO use pra carta/vídeo de aquecimento ou funil (soft-funil), nem pra posicionamento ou conteúdo.
---
...
- **Marketing qualifica, Comercial vende.** Nos funis comerciais (carta, jogadas de campanha, prospecção, mentoria) você fecha o "sim da venda" no 1:1, e o **canal padrão desse 1:1 é a DM/WhatsApp** (fecha no chat com áudio/doc/vídeo curto): a call/reunião é exceção de contexto (ticket muito alto, o lead pede, B2B), o SDR+Closer só entra com equipe e volume. As 4 etapas (abertura/investigação/demonstração/obtenção de compromisso) e a **Conta de Padaria** (diagnóstico pelos números do próprio comprador) → `references/comercial-1a1-e-conta-de-padaria.md`.
- **O canal do fechamento é do FUNIL, não do ticket.** O **Funil de Aula Agendada** fecha DE UMA VEZ no checkout **na própria aula** (one-step); a esteira 1:1 vem DEPOIS como ascensão, e é AÍ que esta skill entra (não no fechamento do produto da aula). Nos funis comerciais, o 1:1 fecha via de regra na DM (acima). Não confunda: aula = one-step no checkout; carta/jogada/prospecção/mentoria = DM por padrão.
...
## Modo DM sem call (fechar high-ticket no chat, sem sessão estratégica, detalhe em `dm-sem-call`)
Quando o lead já chega quente (viu o perfil, consumiu um material) e quer **resolver agora** sem marcar call, você fecha 100% na DM/WhatsApp com áudio, doc e vídeo curto. Comprime a espinha de 7 fases em **5 etapas** (Conexão → Qualificação com 3-4 perguntas que filtram → Autoridade em áudio que normaliza a dor + pede permissão "posso te mostrar como funciona?" → Oferta com doc/vídeo de 3min + "já libero seu acesso" → Follow-up 10min/24h/24h). Doutrina: **venda fácil = entrega fácil**, não mata objeção à força; e **só flexibiliza o preço quando a própria pessoa abre a brecha** ("só teria metade agora" → ajusta a forma de pagamento, nunca baixa o valor cheio). Casa com o Modo A+: a `prospeccao-dm` leva pro pré-qualificador, este modo pega dali quando o lead volta pra fechar. O doc/vídeo da oferta puxa `soft-mentoria` (se for mentoria) ou o bloco Oferta da `soft-posicionamento`.
