...
- **A taxonomia de tipos** (planejamento, não sobe): `entrada` · `engajamento` · `apresentacao` · `dor` · `reação` · `sim-eu-quero` · `numero` · `duvida` · `pergunta` · `objecao` · `atrito` · `anticipacao` · `compra` · `hate` · `prova-real` · `despedida`. Cada um mora numa fase e responde a um gatilho do roteiro (exemplos reais do modelo no §4).
