...
WARN = [
    ...
    # linha ou depois de . ! ?) e polos de 1 palavra: por isso a fala REAL do corpus
...
def _self_test():
    ...
    fakes = [
        'Cinco pessoas, três milhões. Isso é conta, não sorte.',  # a que o corpus pegou
        'Não é sorte, é conta.',
    ...
    # ── falas REAIS do corpus: NÃO podem ser pegas (nem WARN antítese) ───────────
    reais = [
    ...
    for r in reais:
        assert not _warned_antitese(r), f'FALSO-POSITIVO: pegou fala real do corpus: {r!r}'

    print('lint_copy.py self-test OK — HARD (em-dash + travar), WARN (dramática, '
          'antítese-nominal telegráfica, molde "não é sobre") e as falas REAIS do corpus passam limpas.')
